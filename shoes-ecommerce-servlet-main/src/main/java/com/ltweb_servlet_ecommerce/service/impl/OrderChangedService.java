...
public class OrderChangedService implements IOrderChangedService {
    ...
    public void orderBeingDeleted(UserModel userModel, OrderModel orderModel) {
        SendMailUtil.templateWarningOrderChanged(userModel.getUserName(), orderModel.getSlug(), userModel.getEmail(), "Xóa");
    }

    @Override
    public void orderBeingChanged(UserModel userModel, OrderModel orderModel) {
        SendMailUtil.templateWarningOrderChanged(userModel.getUserName(), orderModel.getSlug(), userModel.getEmail(), "Thay đổi");
    }

    @Override
    public void orderBeingChangeStatus(UserModel userModel, OrderModel orderModel) {
        SendMailUtil.templateWarningOrderChanged(userModel.getUserName(), orderModel.getSlug(), userModel.getEmail(), "Thay đổi trạng thái");
    }
}
